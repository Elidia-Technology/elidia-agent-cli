...
class TestNeverPromoteExemption:
    ...
    @pytest.mark.asyncio
    async def test_ordinary_every_time_action_does_auto_promote(self, manager):
        ...
        for _ in range(5):
            await pm.check("code_execute", session_id="s1")
    ...
    @pytest.mark.asyncio
    async def test_db_query_never_auto_promotes_even_after_many_approvals(self, manager):
        pm, approvals = manager
        assert "db_query" in NEVER_PROMOTE

        for _ in range(25):
            allowed = await pm.check("db_query", session_id="s1")
            assert allowed
...
class TestFileReadPathClassification:
    ...
    @pytest.mark.asyncio
    async def test_missing_path_ends_up_at_auto_tier(self, manager):
        pm, approvals = manager
        allowed = await pm.check("file_read", session_id="s1", path=None)
        assert allowed
        assert approvals["count"] == 0, "AUTO tier should never invoke the prompt function"

    @pytest.mark.asyncio
    async def test_external_path_still_requires_confirmation(self, manager, tmp_dir: Path):
        pm, approvals = manager
        pm.set_project_root(tmp_dir)
        allowed = await pm.check("file_read", session_id="s1", path="/etc/passwd")
